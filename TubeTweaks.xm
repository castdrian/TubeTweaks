...
NSBundle *TubeTweaksBundle() {
    static NSBundle *bundle = nil;
    static dispatch_once_t onceToken;
 	dispatch_once(&onceToken, ^{
        NSString *tweakBundlePath = [[NSBundle mainBundle] pathForResource:@"TubeTweaks" ofType:@"bundle"];
        if (tweakBundlePath)
            bundle = [NSBundle bundleWithPath:tweakBundlePath];
        else
            bundle = [NSBundle bundleWithPath:ROOT_PATH_NS(@"/Library/Application Support/TubeTweaks.bundle")];
    });
    return bundle;
}
...
- (void)watchController:(YTWatchController *)watchController didSetPlayerViewController:(YTPlayerViewController *)playerViewController {
    if (playerViewController) {
        ...
        if (!playerViewController.TubeTweaksPanGesture) {
            playerViewController.TubeTweaksPanGesture = [[UIPanGestureRecognizer alloc] initWithTarget:playerViewController
                                                                                               action:@selector(TubeTweaksHandlePanGesture:)];
            playerViewController.TubeTweaksPanGesture.delegate = playerViewController;
            [playerViewController.playerView addGestureRecognizer:playerViewController.TubeTweaksPanGesture];
        }        
    }
    %orig;
}
...
%property (nonatomic, retain) UIPanGestureRecognizer *TubeTweaksPanGesture;
/**
  ...
  */
%new
- (void)TubeTweaksHandlePanGesture:(UIPanGestureRecognizer *)panGestureRecognizer {
...
- (NSMutableArray *)buttons {
    ...
    if (!self.videoPlayerButton || pageStyle != [%c(YTPageStyleController) pageStyle]) {
        ...
        if ([%c(YTPageStyleController) pageStyle]) { //dark mode
            [self.videoPlayerButton setImage:[UIImage imageWithContentsOfFile:[tweakBundle pathForResource:@"TubeTweaksColored-128" ofType:@"png"]] forState:UIControlStateNormal];
        }
        else { // light mode
            [self.videoPlayerButton setImage:[UIImage imageWithContentsOfFile:[tweakBundle pathForResource:@"TubeTweaksColored-128" ofType:@"png"]] forState:UIControlStateNormal];
            // UIImage *image = [UIImage imageWithContentsOfFile:[tweakBundle pathForResource:@"TubeTweaksColored-128" ofType:@"png"]];
...
%ctor {
    ...
    if (![allKeys containsObject:@"YTLPDidPerformFirstRunSetup"]) { 
        ...
        // Set miscellaneous TubeTweaks features to enabled
